...
function TableCard({
  ...
  const handleRemoveFromWaitlist = async (waitlistId: string) => {
    ...
    const confirmed = await showConfirmDialog({
      title: 'Remove Player',
      message: `Remove ${waitlistEntry?.player?.nick || 'this player'} from waitlist?`,
      confirmText: 'Remove',
      cancelText: 'Cancel',
      type: 'warning',
    });
    ...
    setLoading(true);
    try {
      log('Removing player from waitlist:', { waitlistId, tableId: table.id, adminUser });
      await removePlayerFromWaitlist(waitlistId, adminUser);

      // Clean up TC list entry for this player/table
      try {
        const tcList = JSON.parse(localStorage.getItem('tc-list') || '[]');
        const cleaned = tcList.filter((entry: any) => !(entry.playerId === waitlistEntry?.player_id && entry.tableNumber === table.table_number));
  ...
  return (
    <div className={`table-card ${table.status.toLowerCase()} ${tableIsEmpty ? 'empty-table' : ''} ${tableIsFull ? 'full-table' : ''} game-type-${gameTypeClass}${(isPersistent || table.is_persistent) ? ' persistent' : ''}`}>
      ...
      <div className="table-lists">
        ...
        <div
          className={`waitlist-list ${dragOverTarget === 'waitlist' ? 'drag-over' : ''}`}
          onDragOver={(e) => {
            e.preventDefault();
            handleDragOver(e, 'waitlist');
          }}
          onDragLeave={handleDragLeave}
          onDrop={(e) => {
            e.preventDefault();
            handleDrop(e, 'waitlist');
          }}
        >
          <h4>
            Waitlist {draggedPlayer && <span className="drop-hint">(Drop to waitlist)</span>}
            {searchQuery && activeWaitlistPlayers.filter((wl) => {
              const query = searchQuery.toLowerCase();
              return (
                wl.player?.nick?.toLowerCase().includes(query) ||
                wl.player?.name?.toLowerCase().includes(query) ||
                wl.player_id.toLowerCase().includes(query)
              );
            }).length === 0 && activeWaitlistPlayers.length > 0 && (
              <span className="search-no-match"> (No matches)</span>
            )}
          </h4>
          {activeWaitlistPlayers.length === 0 ? (
            <div className="empty-list drop-zone">
              {draggedPlayer ? 'Drop player here to waitlist' : 'No players waiting'}
            </div>
          ) : activeWaitlistPlayers.filter((wl) => {
              if (!searchQuery) return true;
              const query = searchQuery.toLowerCase();
              return (
                wl.player?.nick?.toLowerCase().includes(query) ||
            ...
            activeWaitlistPlayers
              ...
              .map((wl) => {
                const assignment = playerAssignments.get(wl.player_id);
                const selectedEntry = selectedPlayers[wl.player_id];
                const isSelected = selectedEntry?.entryId === wl.id;
                // Don't apply dragging class during drag to avoid interfering with browser drag operation
                return (
                <div
                  key={`waitlist-${wl.player_id}-${wl.id}`}
                  className={`player-item ${isSelected ? 'selected' : ''}`}
                  draggable
                  onDragStart={(e) => handleDragStart(e, wl, table.id, true)}
                  onDragEnd={handleDragEnd}
                ...
                >
                  ...
                  <div className="player-content">
                    <span className="player-name">
                      <span className="player-name-text">
                        {searchQuery ? (
                          <>
                            {(wl.player?.nick || 'Unknown').split(new RegExp(`(${searchQuery})`, 'gi')).map((part, i) => 
                              part.toLowerCase() === searchQuery.toLowerCase() ? (
                                <mark key={i} className="search-highlight">{part}</mark>
                              ) : (
                                <span key={i}>{part}</span>
                          ...
                          wl.player?.nick || 'Unknown'
                        )}
                      </span>
                      {assignment?.seated && (
                        <span className="assignment-icon" title={`Also seated at Table ${assignment.seated}`}>
                          @ T{assignment.seated}
                        </span>
                      )}
                    </span>
                    {wl.called_in && (
                      <div className="called-in-row">
                        <span className="called-in-badge" title="Called in - hasn't paid door fee yet">
                          <span className="called-in-text">Called In</span>
                        </span>
                        <button
                          className="player-pay-seat-btn"
                          onClick={async (e) => {
                            e.stopPropagation();
                            const playerName = wl.player?.nick || 'Unknown';
                            let defaultAmount = 20;
                          ...
                          title="Pay door fee and seat player"
                        >
                          <span className="pay-seat-text">Pay & Seat</span>
                        </button>
                      </div>
                    )}
                  </div>
                  <div className="player-actions">
                    ...
                    <Tooltip content="Table Change - Add to another table's waitlist">
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setTcAddAll(false);
                          setTcSameGameOnly(true);
                          setTableChangeMenu({ player: wl, isFromWaitlist: true });
                        }}
                        title="Table Change"
                      >
                        TC
                    ...
                    <Tooltip content="Remove player from waitlist">
                      <button
                        className="player-quick-remove"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRemoveFromWaitlist(wl.id);
                        }}
                        title="Remove player"
                      >
                        ✕
